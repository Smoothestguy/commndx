CREATE OR REPLACE FUNCTION public.complete_personnel_onboarding(p_token text, p_personnel_id uuid, p_first_name text, p_last_name text, p_email text, p_phone text DEFAULT NULL::text, p_date_of_birth date DEFAULT NULL::date, p_photo_url text DEFAULT NULL::text, p_address text DEFAULT NULL::text, p_city text DEFAULT NULL::text, p_state text DEFAULT NULL::text, p_zip text DEFAULT NULL::text, p_ssn_full text DEFAULT NULL::text, p_citizenship_status text DEFAULT NULL::text, p_immigration_status text DEFAULT NULL::text, p_emergency_contacts jsonb DEFAULT '[]'::jsonb, p_bank_name text DEFAULT NULL::text, p_bank_account_type text DEFAULT NULL::text, p_bank_routing_number text DEFAULT NULL::text, p_bank_account_number text DEFAULT NULL::text, p_direct_deposit_signature text DEFAULT NULL::text, p_tax_classification text DEFAULT NULL::text, p_tax_ein text DEFAULT NULL::text, p_tax_business_name text DEFAULT NULL::text, p_w9_signature text DEFAULT NULL::text, p_w9_certification boolean DEFAULT false, p_ica_signature text DEFAULT NULL::text, p_documents jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF v_token_record IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Invalid, expired, or already used token');
  END IF;

  -- Guard: reject completion when required signatures are missing.
  -- Prevents client sessionStorage restoration from stripping signatures and silently completing onboarding.
  ...
  IF p_w9_signature IS NOT NULL AND p_tax_classification IS NOT NULL THEN
    INSERT INTO personnel_w9_forms (
      ...
      p_tax_ein, p_w9_signature, v_current_time::date,
