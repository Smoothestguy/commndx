...
import require$$1$4, { ipcMain, app, BrowserWindow, shell } from "electron";
...
function initAutoUpdater(win2) {
  ...
  ipcMain.handle("check-for-updates", async () => {
    try {
      return await main$2.autoUpdater.checkForUpdates();
    } catch (err) {
  ...
  ipcMain.handle("download-update", async () => {
    try {
      await main$2.autoUpdater.downloadUpdate();
      return true;
    } catch (err) {
  ...
  ipcMain.handle("install-update", () => {
    main$2.autoUpdater.quitAndInstall(false, true);
  });
  setTimeout(() => {
    main$2.autoUpdater.checkForUpdates().catch((err) => {
