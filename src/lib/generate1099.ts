...
export function generate1099NEC(options: Generate1099Options): jsPDF {
  ...
  if (isVoid) {
    ...
    doc.text("X", formStartX + formWidth - 44.5, y + 2.5);
    doc.setFont("helvetica", "normal");
  }
  ...
  if (isCorrected) {
    doc.setFont("helvetica", "bold");
    doc.text("X", formStartX + formWidth - 24.5, y + 2.5);
    doc.setFont("helvetica", "normal");
  }
  ...
  if (directSales5000Plus) {
    doc.setFont("helvetica", "bold");
    doc.text("X", rightColX + 2.3, rightY + 5.5);
    doc.setFont("helvetica", "normal");
  }
  ...
  if (secondTinNotification) {
    ...
    doc.setFont("helvetica", "bold");
    doc.text("X", rightColX + box2Width + 3.3, rightY + 7);
    doc.setFont("helvetica", "normal");
  }
...
export function downloadForm1099(options: Generate1099Options): void {
