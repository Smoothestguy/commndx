...
interface USANetworkMapFinalProps {
  title?: string;
  subtitle?: string;
  tagline?: string;
  description?: string;
...
const USANetworkMapFinal = ({
  ...
  className,
  showButton = false
}: USANetworkMapFinalProps) => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 });

  // Major US cities positioned to match USA geography (right side focus)
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        gradient.addColorStop(0, 'rgba(255, 230, 180, 0.9)');
      ...
      // Draw city nodes with VERY strong glow effect
      cityPositions.forEach(city => {
        // Extra large outer glow
        const extraGlow = ctx.createRadialGradient(
          city.px, city.py, 0,
          city.px, city.py, city.size * 5
        );
        extraGlow.addColorStop(0, 'rgba(255, 180, 70, 0.7)');
        extraGlow.addColorStop(0.3, 'rgba(255, 150, 50, 0.4)');
        extraGlow.addColorStop(1, 'rgba(255, 140, 30, 0)');
        
        ctx.fillStyle = extraGlow;
        ctx.beginPath();
        ctx.arc(city.px, city.py, city.size * 5, 0, Math.PI * 2);
        ctx.fill();

        // Large outer glow
        const outerGlow = ctx.createRadialGradient(
          city.px, city.py, 0,
          city.px, city.py, city.size * 3.5
        );
        ...
        ctx.beginPath();
        ctx.arc(city.px, city.py, city.size * 3.5, 0, Math.PI * 2);
        ctx.fill();
  ...
  return (
    <div
      ...
      style={{ height }}
    >
      {/* Background: USA night lights map */}
      <div 
        className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-40"
        style={{
          backgroundImage: 'url(/assets/usa-night-lights.jpg)',
...
export default USANetworkMapFinal;
