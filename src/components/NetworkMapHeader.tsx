...
const NetworkMapHeader = ({ 
  ...
  useEffect(() => {
    ...
    // Position camera to focus on USA with a slight delay to ensure globe is ready
    setTimeout(() => {
      if (globeEl.current) {
  ...
  return (
    <div 
      ref={containerRef}
      className={cn("relative w-full overflow-hidden bg-gradient-to-b from-slate-950 to-slate-900", className)}
      style={{ height }}
    >
      {/* 3D Globe Background */}
      <div className="absolute inset-0">
        {dimensions.width > 0 && (
          <Globe
            ...
            pointColor={() => '#ffaa00'}
            pointLabel={(d: any) => `<div style="color: white; background: rgba(0,0,0,0.8); padding: 4px 8px; border-radius: 4px; font-size: 12px;">${d.city}</div>`}
            pointsMerge={true}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
        <div className="text-center px-4 max-w-4xl">
          ...
          <p className="text-lg md:text-xl lg:text-2xl text-blue-100 drop-shadow-lg">
            {subtitle}
          </p>
        </div>
      </div>
