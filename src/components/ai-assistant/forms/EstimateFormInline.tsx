...
interface EstimateFormInlineProps {
...
export function EstimateFormInline({ prefilled, onSubmit, isSubmitting }: EstimateFormInlineProps) {
    ...
    <form onSubmit={handleSubmit} className="mt-3 space-y-4 p-4 bg-muted/30 rounded-lg border border-border/50">
      ...
      >
        {isSubmitting ? (
          ...
          <>Create Estimate — ${subtotal.toFixed(2)}</>
        )}
      </Button>
    </form>
  );
