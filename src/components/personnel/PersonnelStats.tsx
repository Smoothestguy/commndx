...
import { Users, UserCheck, UserX, AlertTriangle, AlertCircle } from "lucide-react";
...
export const PersonnelStats = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex gap-4 overflow-x-auto pb-2 md:grid md:grid-cols-5">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="min-w-[160px] h-24 bg-muted animate-pulse rounded-lg md:min-w-0" />
        ))}
      </div>
    );
  }
  ...
  return (
    <ScrollArea className="w-full">
      <div className="flex gap-4 pb-2 md:grid md:grid-cols-5">
